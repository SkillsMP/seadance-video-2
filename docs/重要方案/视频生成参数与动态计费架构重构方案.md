...
### 21.6 candidates fallback 专项验收

本轮 Phase 3 改造收敛了 image/video 的参数快照与动态计费入口，但不应破坏原有 `candidates` 候选模型 fallback 设计。该机制用于在同一生成请求中支持多个候选 provider/model，当首个 candidate 调用失败时，后端可以继续尝试后续 candidate，从而提升生成成功率。

本专项验收继承《可信模型注册表方案》的 P0 约束：`family / scene / candidates` 必须交叉验证，image/video 公开生成请求必须走 `candidates` 模式。Phase 3 只是把旧的静态 `credits` invariant 升级为 `finalOptions + costCredits` pricing snapshot invariant，不应削弱 fallback 语义。

本专项验收不要求回滚或大改现有实现，重点是确认 Commit 1-4 后，候选模型 fallback 的核心语义仍然成立，并且新增的 `finalOptions / costCredits` 一致性校验没有被绕开。
...
1. image/video generate route 仍然支持遍历多个 `candidates`。
2. first candidate 的 provider 调用失败时，不应直接终止整个请求，而应继续尝试后续 candidate。
3. 只有所有 candidates 都失败时，才返回 `All model candidates failed`。
4. fallback candidate 进入执行链前，必须先与 first candidate 的 pricing snapshot 完成一致性校验。
5. fallback candidate 的 `finalOptions` 与 first candidate 一致时，允许进入后续 provider 调用。
6. fallback candidate 的 `finalOptions` 不一致时，必须由 `assertGenerationPricingConsistency()` 拒绝整个请求。
7. fallback candidate 的 `costCredits` 不一致时，必须由 `assertGenerationPricingConsistency()` 拒绝整个请求。
8. provider generate failed 等单个 candidate 调用错误应被记录到 errors 中，但不应直接中断整个候选链。
9. `validateFallbackCandidateConfigConsistency()` 仍应防止同一 `mediaType + family + scene` 下 candidate 配置漂移。
10. music legacy 路径不参与 image/video candidates fallback 收敛，原有行为应保持不变。

验收方式：

* 优先阅读现有 route 与 pricing snapshot 逻辑，确认 fallback candidate 的执行顺序和一致性约束。
* 若现有测试覆盖不足，只补充轻量测试或脚本断言。
* 不要大改 route 主流程。
* 不要修改 provider fallback 策略。
* 不要改动 pricing snapshot 主链路。
* 不要改动 music legacy 行为。

建议验证命令：

```bash
pnpm ai:validate-models
pnpm tsx scripts/test-calculate-model-credits.ts
pnpm tsx scripts/test-resolve-final-options.ts
pnpm lint
```

验收结论应明确说明：

* candidates fallback 是否仍然可用；
* first candidate 失败后是否会继续尝试后续 candidate；
* finalOptions / costCredits drift 是否仍会被拦截；
* 是否存在需要修复的 High / Medium / Low findings。

### 21.7 moderation 专项验收

Phase 3 收敛的是 image/video 参数快照、动态计费和 candidates fallback，不应把 moderation 问题混进 pricing 或 provider fallback 里处理。本专项只验收生成结果输出审核在 Phase 3 后仍然语义清晰、状态收敛、错误提示准确。

本专项的核心判断：**真实违规是内容安全问题；审核服务请求失败是“结果无法验证”问题。两者都不能展示未审核输出，但不能共用同一个违规文案。**

输入审核仍可遵守全局 fail open / fail closed 配置；输出审核涉及已生成内容是否展示，本专项按 fail closed 处理。也就是说，当 moderation 已启用且输出类型需要审核时，即使全局配置允许 fail open，输出审核 provider 异常也不得放行未验证结果，应进入 `moderation_failed`。
...
1. 输入审核仍然使用服务端 `finalOptions`，candidate fallback 场景同一请求只审核一次。
2. 输出图片 / 视频只有审核 allow 后，才能以 `success` 写入并返回给前端。
3. 审核结果明确 block 时，任务进入 `moderation_blocked`，`taskInfo/taskResult` 只保存脱敏错误信息。
4. Sightengine 400、超时、网络错误、配置缺失、响应结构异常不应映射成内容违规。
5. 审核服务异常在 fail closed 下进入独立的“无法验证”语义，例如 `moderation_failed`，不展示原始 output URL。
6. 输出审核 provider 异常不受全局 fail open 放行；只要 moderation 已启用且输出类型需要审核，就应进入 `moderation_failed`。
7. `moderation_failed` 不复用 provider `failed`，避免误触发失败退款；是否补偿用户应作为独立产品策略处理。
8. `moderation_blocked / moderation_failed` 都应是 query 终态，再次轮询不重复请求 provider，也不重复调用 moderation provider。
9. 前端对 `moderation_blocked` 显示内容安全违规文案；对 `moderation_failed` 显示审核服务暂时不可用 / 结果无法验证文案。
10. Sightengine HTTP 非 2xx 时记录最小诊断日志：status、截断 response body、request mode、provider、checkType；不得记录 API key、secret 或完整签名 URL。
11. 本专项不得修改 pricing snapshot、provider fallback 策略、model registry、music legacy 行为。

验收方式：

* 用安全 prompt 生成图片，确认 output moderation allow 后正常展示。
* 人为 mock Sightengine `output_image` 400，确认前端不显示内容违规文案，任务不暴露 output URL。
* 人为 mock Sightengine block，确认仍进入 `moderation_blocked`。
* 阅读 `updateAITaskById()` 退款条件，确认 `moderation_failed` 不会误走 provider failed 自动退款。

建议验证命令：

```bash
pnpm test:ai-moderation
pnpm lint
```

验收结论应明确说明：

* moderation 问题是否独立于 pricing / candidates；
* Sightengine 400 是否已能定位到 response body；
* 真实违规和审核服务异常是否有不同任务状态和前端文案；
* 是否存在需要修复的 High / Medium / Low findings。

## 22. 2026-06-03 记录：4K 图片 PNG 体积导致输出审核失败的处理思路

背景：

- 当前 4K 图片如果以 PNG 输出，文件体积可能超过 12MB。
- 输出审核链路是 fail closed。生成结果只有通过 output moderation 后才会以 `success` 展示。
- 如果审核 provider 无法下载或解析超大图片，任务会进入 `moderation_failed`，用户看不到生成结果。
- 本问题本质上不是 pricing / candidates fallback 问题，而是“生成输出格式、对象存储元数据、下载体验、审核 provider 输入体积”之间的链路一致性问题。

当前代码观察：

- KIE 图片生成请求已经会透传 `options.output_format`；在已确认 `jpg` 可用后，主要改动在 registry controls/defaults 和链路校验。
- 当前 Nano Banana Pro / Nano Banana 2 的 `output_format` control 只开放 `png`。
- KIE / Fal / Replicate 的 customStorage 图片保存逻辑仍硬编码 `image/png` 和 `.png`。
- 生成页下载文件名当前固定 `.png`；活动页下载扩展名从 URL 路径推断。
- 输出审核使用 `taskInfo.images[].imageUrl`，即审核的是最终暴露给用户的图片 URL。
- 当前 `AIProvider.query()` 只传入 `taskId / mediaType / model`，KIE `queryImage()` 不直接拿得到任务落库的 `finalOptions`。因此本轮不应为了 customStorage 格式推断去改 query 主流程或扩大 provider 接口；KIE customStorage 优先使用 provider 返回结构中可解析的 `output_format`，其次使用输出 URL 后缀。

命名和范围边界：

- 后续产品和文档表述中，`Nano Banana legacy` 这类叫法建议废弃，改成 `Nano Banana Standard` 或“基础版 Nano Banana”。
- 基础版 Nano Banana / Nano Banana Edit 不是旧生成链路；它们仍应走 Phase 3 之后的 `finalOptions + pricing snapshot` 链路。
- “legacy” 这个词只适合描述真正保留旧架构的历史路径，例如当前未纳入 image/video option-aware pricing 的 music 旧路径；不应用来称呼基础版 Nano Banana。
- 本轮 4K PNG 体积修复仍建议只动 Nano Banana Pro / Nano Banana 2，因为当前 4K 体积问题主要来自 Pro / 2 的高分辨率输出。
- Nano Banana Standard / Nano Banana Edit 后续可以在 smoke test 通过后再开放 `output_format`。
- 如果后续 Standard / Edit 开放 JPG，不需要改 pricing；它们仍然保持 fixed 5 credits，只是输出格式多一个可选项。

初步结论：

> 先通过 JPG 输出降低 4K 文件体积，优先避免超大 PNG 进入输出审核链路；再补齐 customStorage MIME / 扩展名 / 下载扩展名一致性；审核专用压缩副本作为后续兜底方案，不作为第一步。

建议执行顺序：

1. 开放 Nano Banana Pro / Nano Banana 2 的 JPG 输出。
2. 本轮先推荐 4K 使用 JPG，不做后端自动切换。当前前端会把控件默认值也提交进 `options`，后端难以可靠区分“用户显式选择”和“默认值带入”；若以后要实现 `resolution === '4K' && output_format 未显式选择 => jpg`，应先补前端 touched state 或调整 options 提交语义。
3. 同步修复 customStorage 的 MIME、扩展名和下载扩展名。不能只改生成参数，否则会出现 JPG 内容被存为 `.png` / `image/png` 的错配。
4. 增加 4K PNG 体积提示，用于减少用户踩坑。本轮不新增服务端 preflight，不拦截用户显式选择 PNG 的请求。
5. 如需边界数据，单独 smoke test Sightengine 对 12MB、15MB、20MB 图片的表现，并记录状态码、错误体和失败阶段。
6. 如后续 GPT Image 2 或更多 4K 模型接入后仍无法控制输出格式，或产品必须保留 PNG 原图，再实现审核专用压缩副本。

实施注意：

- 第 1 步和第 3 步应同批上线，避免格式、MIME、扩展名不一致。
- 需要先确认上游 KIE 对 `output_format` 的合法值是 `jpg`、`jpeg` 还是其它枚举。
- 项目内部 canonical 输出格式建议统一为小写 `png | jpg`。如果 KIE 对某个模型实际要求大小写或枚举差异，只在 KIE adapter 做极小 provider 映射，不把差异暴露给 UI / registry。
- 如果 customStorage 保存 provider 输出，应在 KIE adapter 内保持高内聚、小入侵：若 provider 查询结果中可解析到原始请求参数或 `output_format`，则优先使用；否则从 provider 返回 URL 扩展名推断。`HEAD` 探测不是本轮必须项，只有在 KIE 现有代码结构中很容易实现且不改变 storage 通用层时才考虑；否则回退到已知格式或安全默认值。
- 本轮不为 customStorage 格式推断修改 `/api/ai/query` 主流程、`AIProvider.query()` 类型或 storage 通用层。Fal / Replicate 的 `image/png` 硬编码只记录为后续格式一致性清理，不在本轮扩大。
- 输出审核仍应保持 fail closed；不要为了解决体积问题把 output moderation 改成 fail open。
- 审核专用压缩副本必须保留原图展示与下载语义，不能悄悄替换用户最终资产，除非产品明确接受。

验收重点：

- 4K + JPG 生成的 Nano Banana Pro / Nano Banana 2 图片体积显著低于 PNG。
- `taskInfo.images[].imageUrl` 指向的对象扩展名与真实 `Content-Type` 一致。
- 生成页下载文件扩展名按 blob MIME 推断；活动页本轮保持从 URL 路径推断，不做下载链路重构。
- Sightengine 12MB、15MB、20MB smoke test 如执行，应有明确结论。
- 4K PNG 被用户显式选择时，有明确提示；如果仍超过 Sightengine 限制，应进入 `moderation_failed`，不展示未审核图片，也不显示内容违规文案。
